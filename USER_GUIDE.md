...
**For diners**
- 4-step booking journey (date → time → details → review)
- Real-time availability that respects opening hours and closures
- Section preference: Indoor, Outdoor/Garden, or No Preference
- Email confirmation with a calendar invite and "Add to Google Calendar" button
- 24-hour reminder email
- One-click cancellation via a secure link in the email
...
- Opening-hours editor (per day, per meal period)
...
The public booking page is reached from the **Book a Table** button on the homepage. The whole flow takes about 60 seconds.
...
- Hitting **Confirm Booking** creates the reservation and opens a confirmation page.
...
- A confirmation email arrives within seconds, including:
  ...
  - A calendar invite that imports straight into Apple Calendar / Outlook
...
3. The cancellation page asks for a final confirmation.
4. Once confirmed, the seat is immediately released and the restaurant is notified.

The cancel link is single-use and tied to one booking — it cannot be guessed or reused.

### Adding to Calendar
On the confirmation page:
- **Add to Google Calendar** — opens Google Calendar pre-filled.
- **Download Calendar Invite** — works for Apple Calendar, Outlook and most other calendar apps.
...
Admins log in at the **Admin Login** page with the email and password issued to them.
...
### Manual Booking
For phone-in and walk-in reservations. Identical fields to the public flow, but completed by staff. Useful for repeat regulars who don't want to use the website.
...
Both live on the **Settings** page.
...
Recurring closures support **weekly**, **monthly** and **yearly** patterns. The booking page automatically hides any date that has been closed off, so guests never see a date they can't actually book.

---

## Menu Management

The **Menu** page in the admin area lets staff publish the restaurant menu to the public site.
...
- **Add / Edit / Delete** — full control with instant updates

Changes appear on the public menu the moment they're saved.

---

## Analytics

The **Analytics** page answers the questions managers actually ask:
...
The **Users** page is where the owner manages who can log in.

- **Create** — issue a new admin an email/password login.
...
| Guest creates a booking | Restaurant inbox | Quick summary so the team sees it instantly |
| 24 hours before the booking | Guest | Reminder with date, time and cancel link |
| Guest cancels | Restaurant inbox | Notice that a table has freed up |

Reminders go out automatically every morning — no one has to remember to send them.
...
- Print the run sheet at the start of each service.
...
**For new admins**
- Bookmark the admin dashboard — it's the fastest way back to today's view.
...
The time slot disappears from the booking page. Guests cannot overbook capacity.

**Can we close at short notice?**
Yes — add a **one-time closure** on the Settings page for the affected date. Any bookings for that day stay in the system; cancel them from the bookings list and the guests will be notified by email.
...
Look them up in the bookings list by name or email. From the detail page you can re-send the confirmation or cancel on their behalf.

**Does the system work offline?**
No — it's a live web app and needs internet access on both the guest's device and the admin's device. The trade-off is that everyone is always looking at the same up-to-date information.

**Can guests pay a deposit at booking time?**
Not in this version — payments are handled in person at the restaurant.
